...
export default function PreTest() {
  ...
  const checkPhaseStatus = async () => {
    ...
    if (new Date() > new Date(accountCreated.getTime() + 10 * 24 * 60 * 60 * 1000)) {
      setIsLockedOut(true);
    }
  ...
  const savePreTest = async () => {
    ...
    if (!error) { setHasCompleted(true); alert("✅ Baseline Assessment Locked."); }
    setIsSubmitting(false);
  };

  if (loadingStatus) return <Layout><div className="flex h-screen items-center justify-center font-black animate-pulse text-[#039be5]">VALIDATING SESSION...</div></Layout>;

  return (
    <Layout title="Phase 1: Pre-Test">
      <div className="max-w-[1400px] mx-auto px-6 pb-40">
        
        {/* HEADER SECTION - Based on reference style */}
        <div className="flex flex-col lg:flex-row lg:items-center justify-between py-12 gap-8 border-b border-slate-100 mb-12">
          <div className="space-y-3">
            ...
            <p className="text-slate-500 text-sm max-w-xl">Complete your baseline diagnostic sets to track progress.</p>
          </div>

          <div className="flex items-center gap-4">
            {!hasCompleted ? (
              <button onClick={savePreTest} disabled={isSubmitting} className="flex items-center gap-3 bg-[#039be5] text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg hover:bg-[#01579b] transition-all">
                {isSubmitting ? <RefreshCw className="animate-spin" /> : <ShieldCheck />} Sync Baseline & Lock
              </button>
            ) : (
              <button onClick={() => router.push('/phase/2-weekly-logs')} className="flex items-center gap-3 bg-slate-900 text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl hover:bg-black transition-all">
                Proceed to Weekly Logs <ChevronRight />
              </button>
            )}
          </div>
        </div>

        {/* CARDS GRID - 3 Columns as seen in screenshot */}
            ...
            <div key={ex.id} className={styles.exerciseCard}>
              <div className="flex justify-between items-start mb-6">
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Target: {ex.goal || '8-12 Reps'} • {ex.sets} Sets</p>
                </div>
              ...
              {/* SET INPUTS - Horizontal row layout */}
              <div className="grid grid-cols-3 gap-3 mb-8">
                {[1, 2, 3].map((s) => (
                  <div key={s} className="space-y-2">
                    <label className="text-[9px] font-black text-slate-300 uppercase tracking-tighter block text-center">Set {s}</label>
                    <input 
                      type="number" 
              ...
              {/* CARD FOOTER - Stat summary */}
              <div className="flex items-center justify-between pt-6 border-t border-slate-50">
                <div className="flex flex-col">
                  <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Mean Diagnostic</span>
                  <div className="flex items-baseline gap-1 font-black text-2xl text-slate-900 italic tracking-tighter">
                    {calculateMean(ex.id)} <span className="text-[10px] text-[#039be5]">{ex.unit || 'Reps'}</span>
                  </div>
                </div>
                {results[`${ex.id}_set1`] && <CheckCircle2 size={20} className="text-green-500" />}
              </div>
            </div>
          ))}
        </div>
