...
import { 
  ...
  Zap, ChevronRight, Target, Sparkles, BookOpen, UserCheck
} from 'lucide-react';
  ...
  if (loading) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#f8fafc]">
      <motion.div 
        ...
        className="relative"
      >
        <Loader2 className="text-fbOrange" size={80} />
      </motion.div>
      <motion.p 
        initial={{ opacity: 0 }} animate={{ opacity: 1 }}
        className="mt-6 font-black text-fbNavy uppercase tracking-[0.5em] text-xs"
      >
    ...
    <RoleGuard allowedRole="instructor">
      ...
      >
        ...
        <motion.div 
          ...
          className="relative overflow-hidden bg-fbNavy p-10 md:p-16 rounded-[60px] shadow-2xl shadow-fbNavy/30"
        >
          <motion.div 
            animate={{ opacity: [0.3, 0.6, 0.3], scale: [1, 1.1, 1] }}
            transition={{ duration: 5, repeat: Infinity }}
            className="absolute top-0 right-0 w-2/3 h-full bg-gradient-to-l from-fbOrange/30 to-transparent z-0" 
          />
          ...
          <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
            <div className="space-y-4">
              <div className="inline-flex items-center gap-3 bg-white/10 backdrop-blur-md px-5 py-2 rounded-2xl border border-white/20">
                <BookOpen className="text-fbOrange w-4 h-4" />
              ...
              <h1 className="text-6xl md:text-8xl font-black text-white italic tracking-tighter leading-none">
                TEACHING <span className="text-fbOrange">PULSE</span>
              </h1>
            ...
            <motion.button 
              ...
              whileTap={{ scale: 0.95 }}
              className="group relative bg-white text-fbNavy px-12 py-8 rounded-[35px] font-black text-[13px] uppercase tracking-[0.2em] transition-all overflow-hidden shadow-2xl"
            >
        ...
        {/* --- CORE STATS GRID (Contextualized to Teacher/Student) --- */}
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
          ...
          <motion.div 
            variants={itemVariants}
            className="xl:col-span-8 bg-white p-12 rounded-[70px] shadow-2xl shadow-fbNavy/5 border border-white relative overflow-hidden"
          >
            <div className="flex items-center justify-between mb-16">
              <div>
                <h3 className="text-3xl font-black text-fbNavy uppercase italic tracking-tighter flex items-center gap-4">
                  <BarChart3 className="text-fbOrange" size={32} /> Classroom Density
                </h3>
                <p className="text-slate-400 text-xs font-black uppercase tracking-[0.3em] mt-2">Section Population Analysis</p>
              </div>
              <div className="hidden md:flex gap-3">
                {[1, 2, 3].map(i => <div key={i} className="w-4 h-4 rounded-full bg-slate-100" />)}
              </div>
            </div>
                  ...
                  >
                    <div className="flex justify-between items-center mb-6">
                      <div className="flex items-center gap-5">
                        <div className="w-14 h-14 rounded-[22px] bg-fbNavy text-white flex items-center justify-center font-black italic text-sm group-hover:bg-fbOrange transition-colors">
                          {sec.section_code.substring(0, 2)}
                        </div>
                        <div>
                          <span className="font-black text-fbNavy uppercase text-lg tracking-widest">{sec.section_code}</span>
                          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Assigned Section Code</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <span className="text-2xl font-black text-fbNavy italic">{count}</span>
                    ...
                    <div className="h-4 bg-slate-200/50 rounded-full overflow-hidden">
                      <motion.div 
                        ...
                        className="h-full bg-gradient-to-r from-fbNavy to-blue-500 rounded-full relative group-hover:from-fbOrange group-hover:to-orange-500 transition-all"
                      >
                        <motion.div 
                          animate={{ x: ["0%", "100%"] }}
          ...
          >
            {/* Academic Diversity */}
            <div className="bg-white p-12 rounded-[70px] shadow-2xl shadow-fbNavy/5 border border-white">
              <h3 className="text-2xl font-black text-fbNavy uppercase italic mb-10 flex items-center gap-4">
                <PieChart className="text-fbOrange" size={28} /> Program Split
              </h3>
              <div className="space-y-5">
                {Object.entries(stats.courseData).map(([name, count]) => (
                  <motion.div 
                    ...
                    className="flex items-center justify-between p-6 bg-[#f8fafc] rounded-[35px] border border-transparent hover:border-fbOrange/30 transition-all cursor-default"
                  >
                    <div className="flex items-center gap-5">
                      <div className="w-12 h-12 rounded-2xl bg-white shadow-md flex items-center justify-center">
                        <TrendingUp size={20} className="text-fbOrange" />
                      </div>
                      <span className="text-xs font-black text-fbNavy uppercase tracking-widest">{name}</span>
                    </div>
                    <span className="font-black text-fbNavy italic text-2xl">{count}</span>
                  </motion.div>
                ))}
              </div>
            </div>

            {/* Quick Insights (Teacher Call to Action) */}
            <motion.div 
              whileHover={{ rotate: -2, scale: 1.02 }}
              className="bg-gradient-to-br from-fbOrange to-orange-600 p-12 rounded-[70px] text-white shadow-2xl shadow-fbOrange/40 relative overflow-hidden group"
            >
              ...
              <button className="w-full bg-fbNavy text-white py-6 rounded-[30px] font-black text-[11px] uppercase tracking-[0.3em] flex items-center justify-center gap-4 hover:bg-black transition-all group-hover:gap-6">
...
function StatCard({ title, value, icon, trend, color }) {
  ...
  return (
    <motion.div 
      ...
      className="bg-white p-10 rounded-[55px] border border-slate-100 shadow-2xl shadow-fbNavy/5 relative overflow-hidden group transition-all"
    >
      <div className="relative z-10">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-baseline gap-4">
            <h4 className="text-6xl font-black text-fbNavy italic tracking-tighter group-hover:text-fbOrange transition-colors">
              {value}
            </h4>
