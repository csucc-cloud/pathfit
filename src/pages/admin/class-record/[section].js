...
export default function ClassRecord() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      // section_code is 'text' in your schema
  ...
  const getAvgScore = (studentId, exercise) => {
    ...
    // CRITICAL FIX: Match using exercise.id (ex1, ex2) to match your DB values
    ...
    <div className="min-h-screen bg-[#F8FAFC] p-6 lg:p-10 pb-32">
      <div className="max-w-[1700px] mx-auto flex flex-col md:flex-row justify-between items-center mb-12 gap-6">
        <div className="flex items-center gap-6">
          <button onClick={() => router.back()} className="p-4 bg-white rounded-3xl shadow-sm hover:bg-fbNavy hover:text-white transition-all group">
        ...
        <div className="relative group">
          <label className="absolute -top-3 left-4 bg-[#F8FAFC] px-2 text-[9px] font-black text-fbOrange uppercase tracking-widest z-10">Select Protocol</label>
          <select 
            value={selectedActivity}
            onChange={(e) => setSelectedActivity(e.target.value)}
            className="appearance-none bg-white text-fbNavy px-8 py-5 rounded-[24px] font-black text-[12px] uppercase outline-none cursor-pointer border border-gray-100 shadow-xl min-w-[280px]"
          >
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50">
                  <th className="p-8 text-[11px] font-black uppercase text-fbNavy sticky left-0 bg-slate-50 border-r border-gray-100"> Operative Name</th>
                  {PATHFIT_EXERCISES.map((ex) => (
                    <th key={ex.id || ex.name} className="p-4 text-[9px] font-black uppercase text-center text-gray-400 min-w-[100px]">{ex.name}</th>
                  ))}
                  ...
                  students.map(s => {
                    ...
                    return (
                      <tr key={s.id} className="hover:bg-fbGray/10 transition-colors group">
                        <td className="p-8 sticky left-0 bg-white group-hover:bg-gray-50 font-black text-xs text-fbNavy border-r italic uppercase">{s.full_name || 'Unknown Student'}</td>
                        {scores.map((score, i) => (
                        ...
                        <td className="p-6 text-center text-sm font-black text-fbOrange bg-fbOrange/5">{(rowTotal / PATHFIT_EXERCISES.length).toFixed(1)}</td>
                      </tr>
                    );
                  })
                ) : (
                  <tr>
                    <td colSpan={PATHFIT_EXERCISES.length + 3} className="p-20 text-center text-gray-400 font-bold uppercase text-xs">
                      {loading ? <Loader2 className="animate-spin mx-auto text-fbOrange" size={32} /> : 'No Operatives found in this section'}
                    </td>
                  </tr>
                )}
              </tbody>
