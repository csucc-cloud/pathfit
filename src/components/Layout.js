...
const Layout = ({ children }) => {
  ...
  return (
    <div className="flex min-h-screen bg-fbGray text-fbNavy font-sans selection:bg-fbOrange/20 overflow-x-hidden">
      <aside className={`bg-fbNavy text-white hidden md:flex flex-col fixed h-full z-30 shadow-[10px_0_30px_rgba(0,0,0,0.05)] transition-all duration-500 cubic-bezier(0.4, 0, 0.2, 1) ${isSidebarCollapsed ? 'w-24' : 'w-72'}`}>
        <div className="p-8 flex items-center justify-between border-b border-white/5">
          {!isSidebarCollapsed && (
            <div className="flex items-center gap-3 group">
              <div className="w-10 h-10 bg-gradient-to-br from-fbOrange to-orange-600 rounded-xl rotate-3 shadow-lg shadow-fbOrange/25 flex items-center justify-center font-black text-white transition-transform group-hover:rotate-0 group-hover:scale-110 duration-300">P</div>
              <div className="flex flex-col">
                <h1 className="text-lg font-black tracking-tighter leading-none uppercase">PATHFit <span className="text-fbAmber">Portal</span></h1>
                <span className="text-[9px] font-bold text-fbOrange/60 tracking-[0.2em] uppercase mt-1">Academic Suite</span>
              </div>
            </div>
          )}
          <button onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)} className={`p-2.5 bg-white/5 hover:bg-white/10 rounded-xl transition-all ${isSidebarCollapsed ? 'mx-auto' : ''}`}>
        ...
        <nav className="flex-1 px-4 space-y-1.5 mt-8 overflow-y-auto custom-scrollbar">
          {menuItems.map((item) => {
            ...
            const isActive = item.name === 'Dashboard' ? router.asPath === '/dashboard' : router.asPath === item.path;
            ...
            return (
              ...
              >
                <div className={`flex items-center gap-4 p-3.5 rounded-2xl transition-all duration-300 group relative ${isActive ? 'bg-gradient-to-r from-fbOrange to-fbOrange/80 text-white shadow-xl shadow-fbOrange/20' : 'hover:bg-white/5 text-gray-400 hover:text-white'} ${isSidebarCollapsed ? 'justify-center' : ''} ${itemLocked ? 'opacity-40 cursor-not-allowed' : 'cursor-pointer'}`}>
                  <div className={`p-1 rounded-lg transition-colors ${isActive ? 'bg-white/20' : 'bg-transparent'}`}>
                    <Icon size={20} className={`${isActive ? 'animate-pulse' : ''}`} />
                  </div>
                  {!isSidebarCollapsed && <span className="font-bold text-[13px] tracking-wide flex-1">{item.name}</span>}
                  {itemLocked && !isSidebarCollapsed && <Lock size={12} className="opacity-40" />}
                  {isActive && !isSidebarCollapsed && <div className="w-1.5 h-1.5 bg-white rounded-full animate-bounce" />}
                  {isSidebarCollapsed && (
        ...
        <div className="p-6">
          <button onClick={handleSignOut} className="w-full flex items-center gap-4 text-gray-500 p-4 hover:bg-red-500/10 hover:text-red-400 rounded-[1.5rem] transition-all font-black text-xs group">
      ...
      <header className="md:hidden fixed top-0 w-full bg-fbNavy/80 backdrop-blur-xl p-4 flex justify-between items-center text-white z-40 border-b border-white/5 shadow-lg">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-black text-sm uppercase tracking-tighter">PATHFit <span className="text-fbAmber">Portal</span></h1>
        </div>
        <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="p-2.5 bg-white/5 rounded-xl active:scale-90 transition-all">
          {isMobileMenuOpen ? <X size={20} /> : <Menu size={20} />}
        </button>
      </header>

      <div className={`md:hidden fixed inset-0 z-50 transition-all duration-500 ${isMobileMenuOpen ? 'translate-x-0 opacity-100' : '-translate-x-full opacity-0'}`}>
        ...
        <nav className="relative w-[85%] h-full bg-fbNavy p-8 pt-24 space-y-6 shadow-2xl border-r border-white/5">
          <div className="mb-10"><span className="text-fbOrange text-[10px] font-black uppercase tracking-[0.3em]">Navigation</span></div>
          {menuItems.map((item) => {
            const Icon = item.icon;
            const itemLocked = item.requiresUnlock && isLocked;
            const isActive = item.name === 'Dashboard' ? router.asPath === '/dashboard' : router.asPath === item.path;
            return (
              <Link key={item.path} href={itemLocked ? '#' : item.path} onClick={(e) => { if(itemLocked) e.preventDefault(); else setIsMobileMenuOpen(false); }}>
                <div className={`flex items-center gap-5 p-5 rounded-[2rem] font-black transition-all ${isActive ? 'bg-fbOrange text-white scale-[1.02]' : 'bg-white/5 text-white/50'} ${itemLocked ? 'opacity-30' : ''}`}>
          ...
          <div className="pt-10 mt-10 border-t border-white/5">
            <button onClick={handleSignOut} className="w-full flex items-center gap-5 text-red-400 p-5 rounded-[2rem] bg-red-500/10 font-black">
      ...
      <main className={`flex-1 flex flex-col min-h-screen transition-all duration-500 pt-20 md:pt-0 ${isSidebarCollapsed ? 'md:ml-24' : 'md:ml-72'}`}>
        <div className="flex-1 relative pb-10">
          {isPageLoading ? <UniversalSkeleton /> : (
      ...
      <style jsx global>{`
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb:hover { background: rgba(255, 107, 0, 0.2); }
      `}</style>
    </div>
  );
};
